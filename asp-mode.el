...
;  (insert (regexp-opt '("if" "sub" "function" "then" "else" "end" "while" "do" "for" "each" "in" "set" "wend" "next" "loop" "call" "option" "explicit" "redim" "preserve" "dim" "const" "to") t))
  (list
   '("\\<\\(c\\(?:all\\|onst\\)\\|d\\(?:im\\|o\\)\\|e\\(?:ach\\|lse\\|nd\\|xplicit\\)\\|f\\(?:or\\|unction\\)\\|i[fn]\\|loop\\|next\\|option\\|preserve\\|redim\\|s\\(?:et\\|ub\\)\\|t\\(?:hen\\|o\\)\\|w\\(?:end\\|hile\\)\\)\\>" . font-lock-keyword-face))
